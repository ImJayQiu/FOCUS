...
class Forecastm2Controller < ApplicationController
	...
	@final_output.each do |final| # list output of each forecast method
		...
		@nc_vars[0].split(" ").each do |var|
		    ...
		    step2 = "gmt pscoast #{gmtRR} #{gmtJJ} -B0 -Df -N3/2p -W1p -Slightblue -A10000 -K -O >> #{gmtNAME_raw}.ps"
	...
	@output_dir = output_dir = path[0..-2].join("/")
	...
	Dir.chdir(output_dir) do
	    ...
	    ######### grid only plot  ################
	    cp1 = "gmt grdimage #{below_NC} #{gmtRR} -C#{below_CPT} #{gmt_rotate} -Q -JX6i+ -I+ -K > cp_#{gmtNAME}.ps"
	    cp2 = "gmt grdimage #{avg_NC} #{gmtRR} -C#{avg_CPT} #{gmt_rotate} -Q -JX6i+ -I+ -K -O >> cp_#{gmtNAME}.ps"
	    cp3 = "gmt grdimage #{above_NC} #{gmtRR} -C#{above_CPT} #{gmt_rotate} -Q -JX6i+ -I+ -O >> cp_#{gmtNAME}.ps"
	    cp4 = "gmt psconvert cp_#{gmtNAME}.ps -Tg -A"
